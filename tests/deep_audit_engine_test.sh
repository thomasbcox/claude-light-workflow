...
echo "== AC2: source-identity gate (deferred AC c; AR-1 one fingerprint, dirty executable) =="
has "source-identity gate present" "$SKILL" "Source-identity gate"
has "recompute + compare source.contentFingerprint" "$SKILL" "source.contentFingerprint"
has "one mechanism, no dirty-plan refusal (AR-1)" "$SKILL" "no dirty-plan refusal"
has "excluding reviews stops self-invalidation" "$SKILL" "self-invalidat"
has "fingerprint is NUL-safe over tracked files (RR2-3)" "$SKILL" "ls-files -z"
has "fingerprint reads working-tree exec bit, catches unstaged chmod (RR2-3)" "$SKILL" "unstaged chmod registers"
...
has "executability enforces estTokens = runs × tokensPerRun (RR2-2)" "$SKILL" "estTokens = runs × tokensPerRun"

echo "== AC4: run manifest -> fleet (DR-3) =="
has "run manifest section" "$SKILL" "Run manifest"
has "DR-3 cited" "$SKILL" "DR-3"
has "runId carries full row identity (RR2-1)" "$SKILL" "<lens>::<altitude>::<scope>::<unitId>::p<pass>"
...
has "manifest est-cost recomputed from constant, not tautology (RR2-2)" "$SKILL" "manifest-record cost sum"
...
echo "== AC5: adversarial verify — mechanical confirmation + honest judgment (DR-1, AR-2) =="
...
has "no blanket-deterministic claim (AR-2)" "$SKILL" "blanket-deterministic"
has "mechanical tier deterministic where it fires" "$SKILL" "deterministic where it fires"
has "judgment tier honest, explicitly not deterministic" "$SKILL" "NOT claimed deterministic"
has "default refuted on uncertainty" "$SKILL" "defaults to REFUTED"
has "echo chamber closed upstream, not at adjudicator" "$SKILL" "is **upstream**"
has "promoted only if its adjudication tier promotes" "$SKILL" "adjudication tier promotes"
...
echo "== AC7: failure != planned omission (+ durable failure record, AR-3) =="
has "failure vs planned omission invariant" "$SKILL" "failure ≠ planned omission"
has "failure writes no findings report" "$SKILL" "writes no findings report"
has "partial sweep never presented as whole" "$SKILL" "partial sweep"
has "durable failure diagnostic (AR-3)" "$SKILL" "failure is not silent"
has "failed-run artifact path" "$SKILL" "reviews/audit-run-"
has "failed midway vs never began" "$SKILL" "never began"

echo "== AC8: contract handling (extends v1 in place, AR-1) =="
has "contract handling section" "$SKILL" "Contract handling"
has "extends plan-schema v1 in place" "$SKILL" "v1 in place"
...
has "report: source echoes contentFingerprint (AR-1)" "$RSCHEMA" '"contentFingerprint"'
