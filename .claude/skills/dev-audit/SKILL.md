...
| Markdown / docs — `*.md` | markdownlint, lychee (links) | `markdownlint` (no `--fix`); `lychee` (read-only) — **link check is advisory** (network-dependent; prefer scheduled over a blocking gate) | — | — | — |
