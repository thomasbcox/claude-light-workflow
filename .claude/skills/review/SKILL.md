...
2. **Build note.** Append to `reviews/<slug>.md` a `## Build note (<date>)` section: the AC→file map and gate result. Do not include a `git diff --stat` block — the stat is derivable from git and causes self-referential staleness when the story file is part of the diff.
