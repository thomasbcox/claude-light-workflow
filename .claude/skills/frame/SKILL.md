...
9. **Implement** the spec **AC by AC**, building the **approved shape** (apply the design decisions; do not silently re-litigate them) — minimum change, no gold-plating. For each AC whose test notes name the gate as the oracle, **demonstrate red before done**: apply the planned regression (temporarily), run the gate, observe the named check fail, revert, and record the result in the story file's Test notes. A planned check that cannot be made to go red is a **dead assertion** — stop and fix the test, not the note. A check added during implementation carries the same obligation. Never quietly reshape a planned check to match what the code happens to cover; if a check was wrong, say so in the Test notes and say why. Commit on the feature branch. When done and committed, tell Thomas to run `/review` — and, because this is a **first** review, remind him the approach pass runs by default and he can scope it: **`/review approach`** (force the approach pass) or **`/review correctness`** (skip straight to the line-level pass).
