...
5. **Draft the spec** into `reviews/<slug>.md`:
   ...
   - `## Test notes` — how each AC will be checked, **plus a falsification plan**: for each AC, name at least one plausible regression that must be caught **and the oracle mode that catches it** — `gate` (an executable check goes red), `manual` (a named inspection), or `reviewer` (judgment at a review altitude). Derive each entry from the **criterion**, not from any intended test or assertion (none exists yet — that is the point); reserve "the gate goes red" for `gate` oracles. For any AC asserting **presence or shape**, include the case where the element renders *nothing*. If the design sketch is `N/A — mechanical`, the plan may be `N/A — <one-line reason>`; silence is not an option. For a scope-containment AC (one that limits which files the diff may touch), do **not** restate a file count ("must show only N files") — that duplicates the AC's file list and goes stale on any scope change. Instead say: run `git diff --name-only <baseBranch>...HEAD` and verify no files appear beyond those the AC enumerates.
